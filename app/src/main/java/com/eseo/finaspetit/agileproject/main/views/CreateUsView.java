...
public class CreateUsView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.setTitle(getResources().getString(R.string.actionBar_create_us_activity_title));
        ...
        binding.button3.setOnClickListener(v -> {
            if(binding.editTextDescUS.getText().toString().length() != 0 && binding.editTextNameUS.getText().toString().length() != 0){
                US usToAdd=new US("",binding.editTextNameUS.getText().toString(),binding.editTextDescUS.getText().toString(),false,Timestamp.now(),getResources().getString(R.string.state_CREATED),currentSaloon.getId());
                ddb.addUSToSalon(usToAdd);
                Toast.makeText(CreateUsView.this,
                        getResources().getString(R.string.msg_create_us_success),
